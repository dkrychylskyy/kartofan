...
kartofan.controller('mapActivityCtrl', ['NgMap', '$modal', '$popover', '$scope', '$filter', function mapActivityCtrl(NgMap, $modal, $popover, $scope, $filter) {
    ...
    */
    ...
    /* Les donnees */
    vm.markersList = [];
    var marker1 = new Restauant(1, [43.65095096, 1.38046211], "HolaPizza", "pizza", "../img/rest1.jpg", 10, 13, menu);
    ...
    vm.filterType = function () {
        ...
        console.log(vm.filter.prix);
    };

    vm.filterLivraison = function () {
        var filter = vm.filter;
        var min = vm.filter.delaiLivraison;
        if (min == null) {
            filter = undefined;
        }
        vm.markers = $filter('filter')(vm.markersList, vm.filter);
